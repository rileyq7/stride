"""
ASICS product scraper for tech specs.

Site: https://www.asics.com
Uses Playwright for bot protection bypass.
Discovers ALL running shoes from ASICS catalog pages.
"""
...
class AsicsScraper(PlaywrightBrandScraper):
    """Scraper for ASICS product specifications using Playwright."""

    BRAND_NAME = 'ASICS'
    BASE_URL = 'https://www.asics.com'

    # ASICS catalog pages for ALL running shoes
    CATALOG_URLS = [
        # Men's Running
    ...
    async def discover_all_products(self) -> List[str]:
        """Crawl ASICS catalog pages to discover ALL product URLs."""
        ...
        for catalog_url in self.CATALOG_URLS:
            ...
            try:
                ...
                product_links = soup.select('a[href*="/p/"], .product-tile a[href]')

                for link in product_links:
                    href = link.get('href', '')
                    if '/p/' in href or 'product' in href.lower():
                        if href.startswith('/'):
        ...
        logger.info(f"Total unique ASICS products discovered: {len(all_urls)}")
    ...
    async def scrape_product_specs_async(self, product_url: str) -> Optional[ProductSpecs]:
        """Scrape ASICS product page for specs."""
        ...
        try:
            ...
            logger.error(f"Error scraping ASICS product: {e}")
            return None
    ...
    def _extract_product_details(self, soup, html: str, specs: ProductSpecs):
        ...
        # Cushion
        if 'ff blast' in full_text or 'flytefoam blast' in full_text:
            specs.cushion_type = 'FF Blast'
            specs.cushion_level = 'max'
        elif 'flytefoam' in full_text:
            specs.cushion_type = 'FlyteFoam'
            specs.cushion_level = 'moderate'
        elif 'gel' in full_text:
    ...
    def _detect_category(self, specs: ProductSpecs):
        ...
        # Stability
        if any(model in name_lower for model in ['kayano', 'gt-2000', 'gt-1000']):
            specs.subcategory = 'stability'
        # Neutral
        elif any(model in name_lower for model in ['nimbus', 'cumulus', 'novablast']):
            specs.subcategory = 'neutral'
        # Racing
        elif any(model in name_lower for model in ['metaspeed', 'magic speed', 'superblast']):
