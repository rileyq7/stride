"""
Saucony product scraper for tech specs.

Site: https://www.saucony.com
Uses Playwright for bot protection bypass.
Discovers ALL running shoes from Saucony catalog pages.
"""
...
class SauconyScraper(PlaywrightBrandScraper):
    """Scraper for Saucony product specifications using Playwright."""

    BRAND_NAME = 'Saucony'
    BASE_URL = 'https://www.saucony.com'

    # Saucony catalog pages for ALL running shoes
    CATALOG_URLS = [
        # Men's Road Running
    ...
    async def discover_all_products(self) -> List[str]:
        """Crawl Saucony catalog pages to discover ALL product URLs."""
        ...
        for catalog_url in self.CATALOG_URLS:
            ...
            try:
                ...
                product_links = soup.select('.product-tile a[href], a[href*="/shoes/"]')

                for link in product_links:
                    href = link.get('href', '')
                    if '/shoes/' in href or '/product/' in href:
                        if href.startswith('/'):
        ...
        logger.info(f"Total unique Saucony products discovered: {len(all_urls)}")
    ...
    async def get_product_url_async(self, shoe_name: str) -> Optional[str]:
        """Search Saucony for a product."""
        search_query = shoe_name.replace(' ', '+')
        search_url = f"{self.BASE_URL}/en/search?q={search_query}"
    ...
    async def scrape_product_specs_async(self, product_url: str) -> Optional[ProductSpecs]:
        """Scrape Saucony product page for specs."""
        ...
        try:
            ...
            logger.error(f"Error scraping Saucony product: {e}")
            return None
    ...
    def _extract_product_details(self, soup, html: str, specs: ProductSpecs):
        ...
        # Cushion
        if 'pwrrun pb' in full_text:
            specs.cushion_type = 'PWRRUN PB'
        ...
        if 'speedroll' in full_text or 'carbon' in full_text:
            specs.has_carbon_plate = True
            specs.has_rocker = 'speedroll' in full_text
    ...
    def _detect_category(self, specs: ProductSpecs):
        ...
        # Stability
        if any(model in name_lower for model in ['guide', 'hurricane', 'omni']):
            specs.subcategory = 'stability'
        # Neutral
        elif any(model in name_lower for model in ['triumph', 'ride', 'kinvara']):
            specs.subcategory = 'neutral'
        # Racing
        elif any(model in name_lower for model in ['endorphin pro', 'endorphin elite']):
