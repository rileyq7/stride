"""
New Balance product scraper for tech specs.

Site: https://www.newbalance.com
Uses Playwright for bot protection bypass.
Discovers ALL running/basketball shoes from New Balance catalog pages.
"""
...
class NewBalanceScraper(PlaywrightBrandScraper):
    """Scraper for New Balance product specifications using Playwright."""

    BRAND_NAME = 'New Balance'
    BASE_URL = 'https://www.newbalance.com'

    # New Balance catalog pages for running and basketball shoes
    CATALOG_URLS = [
        # Men's Running
    ...
    async def discover_all_products(self) -> List[str]:
        """Crawl New Balance catalog pages to discover ALL product URLs."""
        ...
        for catalog_url in self.CATALOG_URLS:
            ...
            try:
                ...
                product_links = soup.select('.product-tile a[href], a[href*="/product/"], a[href*="/pd/"]')

                for link in product_links:
                    href = link.get('href', '')
                    if '/product/' in href or '/pd/' in href:
                        if href.startswith('/'):
        ...
        logger.info(f"Total unique New Balance products discovered: {len(all_urls)}")
    ...
    async def get_product_url_async(self, shoe_name: str) -> Optional[str]:
        """Search New Balance for a product."""
        name_lower = shoe_name.lower().strip()
        name_lower = name_lower.replace('new balance ', '').replace('nb ', '')
    ...
    async def scrape_product_specs_async(self, product_url: str) -> Optional[ProductSpecs]:
        """Scrape New Balance product page for specs."""
        ...
        try:
            ...
            logger.error(f"Error scraping New Balance product: {e}")
            return None
    ...
    def _detect_category(self, specs: ProductSpecs):
        ...
        # Stability
        if any(model in name_lower for model in ['860', 'vongo', '1540']):
            specs.subcategory = 'stability'
        # Racing
        elif any(model in name_lower for model in ['fuelcell rc', 'fuelcell supercomp', 'sc elite']):
            specs.subcategory = 'racing'
            specs.has_carbon_plate = True
        # Neutral
        elif any(model in name_lower for model in ['1080', '880', 'rebel', 'propel']):
