"""
Mizuno product scraper for tech specs.

Site: https://www.mizunousa.com
Uses Playwright for bot protection bypass.
Discovers ALL running shoes from Mizuno catalog pages.
"""
...
class MizunoScraper(PlaywrightBrandScraper):
    """Scraper for Mizuno product specifications using Playwright."""

    BRAND_NAME = 'Mizuno'
    BASE_URL = 'https://www.mizunousa.com'

    # Mizuno catalog pages for ALL running shoes
    CATALOG_URLS = [
        # Men's Running
    ...
    async def discover_all_products(self) -> List[str]:
        """Crawl Mizuno catalog pages to discover ALL product URLs."""
        ...
        for catalog_url in self.CATALOG_URLS:
            ...
            try:
                ...
                product_links = soup.select('.product-tile a[href], a[href*="/product/"], a[href*=".do"]')
        ...
        logger.info(f"Total unique Mizuno products discovered: {len(all_urls)}")
    ...
    async def get_product_url_async(self, shoe_name: str) -> Optional[str]:
        """Search Mizuno for a product."""
        name_lower = shoe_name.lower().strip()
        name_lower = name_lower.replace('mizuno ', '')
    ...
    async def scrape_product_specs_async(self, product_url: str) -> Optional[ProductSpecs]:
        """Scrape Mizuno product page for specs."""
        ...
        try:
            ...
            logger.error(f"Error scraping Mizuno product: {e}")
            return None
    ...
    def _extract_product_details(self, soup, html: str, specs: ProductSpecs):
        ...
        if 'wave plate' in full_text or 'mizuno wave' in full_text:
    ...
    def _detect_category(self, specs: ProductSpecs):
        ...
        # Stability
        if any(model in name_lower for model in ['wave inspire', 'wave horizon', 'wave paradox']):
            specs.subcategory = 'stability'
        # Neutral
        elif any(model in name_lower for model in ['wave rider', 'wave sky', 'wave neo']):
            specs.subcategory = 'neutral'
        # Racing
        elif any(model in name_lower for model in ['wave rebellion', 'wave duel']):
            specs.subcategory = 'racing'
        # Trail
        elif any(model in name_lower for model in ['wave mujin', 'wave daichi']):
            specs.terrain = 'trail'
